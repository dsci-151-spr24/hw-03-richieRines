...
***Respondents could answer with Yes, No, or Does not apply***
...
```{r}
gss16 %>%
  filter(!(harass5 == "Does not apply (i do not have a job/superior/co-worker)")) %>%
  nrow()
```

***It is applicable to 48% of respondents***
...
```{r}
gss16 %>%
  summarise(mean = mean(email, na.rm = TRUE),
            median = median(email, na.rm = TRUE)
            )
```

***The mean average is 417 minutes and the median is 120 minutes. The mean is the better measure because it takes the average amount of minutes rather than the most common.***
...
***The possible responses are Working fulltime (1321), Retired (574), Working parttime (345), Keeping house (284), Unempl, laid off (118), Other (89), School (76), and Temp not working (57).***
...
```{r}
#unsure if this is a good way to do it but I think it works

email_augment <- augment(email_fit$fit)
...
```{r}
gss16 <- gss16 %>%
  mutate(advfront_agree = case_when(
...
```{r}   
gss16 <- gss16 %>%
  mutate(beliefs = recode(polviews,
...
```{r}
gss16 %>%
  ggplot(aes(x = beliefs, fill = advfront_agree)) +
  geom_bar() +
  labs(title = "Beliefs (Liberal/Conservative) and Support for Science", x = "Belief", fill = "Support Science")
```
